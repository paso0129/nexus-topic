...
톤 수정 규칙 (가장 중요):
1. 절대 1인칭(나, 저, 내가, 필자, 우리) 사용 금지 → 3인칭 객관적 서술로 변환
2. "당신", "여러분" 등 독자 직접 호칭 금지
3. 감정적/선정적 표현 제거: "불편한 진실", "충격적", "놀라운", "축배를 들기엔" 등
4. 개인 의견 → 데이터/전문가 의견으로 전환 (예: "나는 동의할 수 없다" → "이에 대해 전문가들은 다른 시각을 제시한다")
5. 수사적 질문은 유지 가능하되 1인칭 포함된 것은 제거
